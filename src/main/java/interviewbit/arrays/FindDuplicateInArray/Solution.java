package interviewbit.arrays.FindDuplicateInArray;
...
public class Solution {
	...
	public int repeatedNumber(final List<Integer> a) {
		...
		int numRanges = ((N - 1) / root) + 1;
		for (int i = 0; i < numRanges && found == -1; i++) {
			if (i < numRanges - 1 || N % root == 0) {
				if (map1[i] > root) found = i;
			} else {
				if (map1[i] > N % root) found = i;
			}
		}
		if(found == -1)
		{
		...
		HashMap<Integer, Boolean> map = new HashMap(root);
		for(int i=0; i<a.size(); i++)
		{
			if((a.get(i)-1)/root == found)
			{
	...
	public static void main(String[] args)
	{
		...
		System.out.println(sol.repeatedNumber(list));
	}
}
